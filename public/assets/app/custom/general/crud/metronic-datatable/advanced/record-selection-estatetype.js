...
var KTDatatablelocation = function() {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/estatetype/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
           /*  {
            ...
            },
            {
                ...
                template: function(row) {
                    return '\
                    \<a href="' + location.protocol + "//" + window.location.host + '/estatetype/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var locationSelector = function() {
        ...
        var datatable = $('#estatetypeRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.ktDelete', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/estatetype/delete/${elemID}`;
                }
            });
        });
        ...
        $(document).on('click', '.KTMembershipStatusUpdate', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/estatetype/status-change/${elemID}`;
                }
            });
        });
    };
