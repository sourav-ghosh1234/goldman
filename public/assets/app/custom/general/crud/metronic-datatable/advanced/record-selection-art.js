...
var KTDatatableCms = function() {
	...
	var options = {
		...
		data: {
			...
			source: {
				read: {
					url: `${location.protocol}//${window.location.host}/art/getall`,
				},
			},
			pageSize: 10,
			serverPaging: true,
		...
		columns: [
            {
				...
				template: function(row){
					return `<img src="/uploads/art/${row.image}" alt="" height="80" width="80">`
				},
            },
			{
				field: 'title',
			...
			},
			{
				...
				template: function(row) {
					return '\
						\<a href="http://'+window.location.host+'/art/edit/'+row._id+'" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var cmsSelector = function() {
        ...
        var datatable = $('#artRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.ktManagerDelete', function(){
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/art/delete/${elemID}`;
                }
            });
        }); 
        ...
        $(document).on('click', '.KTLanguageStatusUpdate', function(){
            ...
            swal.fire({
               ...
               // text: "You won't be able to revert this!",
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/art/status-change/${elemID}`;
                }
            });
        })
    };
