"use strict";
// Class definition
var KTDatatableContcatUs = function() {
    // Private functions
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/contactus/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
    ...
    // basic demo
    var contactusSelector = function() {

        options.search = {
            input: $('#generalSearch'),
        };

        var datatable = $('#contactusRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.ktManagerDelete', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/contactus/delete/${elemID}`;
                }
            });
        });
        ...
        $(document).on('click', '.KTLanguageStatusUpdate', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/contactus/status-change/${elemID}`;
                }
            });
        })
    };
