...
var KTDatatableCms = function() {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/city/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
            ...
            },
            {
                ...
                width: 110,
                template: function(row) {
                    return row.countryDetails[0].country_name;
                },
            },
            {
                field: 'status',
            ...
            },
            {
                ...
                template: function(row) {
                    return '\
						\<a href="http://' + window.location.host + '/city/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
						<i class="flaticon-edit"></i>\
						</a>\
    ...
    var cmsSelector = function() {
        ...
        var datatable = $('#cityRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.ktManagerDelete', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/city/delete/${elemID}`;
                }
            });
        });
        ...
        $(document).on('click', '.KTLanguageStatusUpdate', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/city/status-change/${elemID}`;
                }
            });
        })
    };
