...
var KTDatatablelocation = function() {
    ...
    var options = {
        ...
        columns: [
            ...
            //     template: function(row) {
            //         return '\
            //         \<a href="' + location.protocol + "//" + window.location.host + '/transaction/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var locationSelector = function() {
        ...
        $(document).on('click', '.KTMembershipStatusUpdate', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/transaction/status-change/${elemID}`;
                }
            });
        });
    };
