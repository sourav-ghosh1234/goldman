...
var KTDatatablelocation = function() {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/revenue/getall`,
                    params:{type:utype}
                },
            },
            pageSize: 10,
        ...
        columns: [
            ...
            //         return moment(row.payment_date).format('LLL');
            //     },
            // },
            {
                field: 'user_type',
            ...
            },
            {
                ...
                width: 150,
                template: function(row) {
                    return row.payment_received_amount;
                },
            },
            // {
            //     field: 'status',
            ...
            //     template: function(row) {
            //         return '\
            //         \<a href="' + location.protocol + "//" + window.location.host + '/revenue/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var locationSelector = function() {
        ...
        var datatable = $('#revenueRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.ktDelete', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/revenue/delete/${elemID}`;
                }
            });
        });
        ...
        $(document).on('click', '.KTMembershipStatusUpdate', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/revenue/status-change/${elemID}`;
                }
            });
        });
    };
