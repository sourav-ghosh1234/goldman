...
var KTDatatableAgency = function() {
    ...
    var options = {
        ...
        data: {
            ...
            source: {
                read: {
                    url: `${location.protocol}//${window.location.host}/member/getall`,
                },
            },
            pageSize: 10,
            serverPaging: true,
        ...
        columns: [
           ...
           /*  {
            ...
            },
            {
                ...
                template: function(row) {
                    return '\
                    \<a href="' + location.protocol + "//" + window.location.host + '/member/edit/' + row._id + '" class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="Edit">\
    ...
    var agencySelector = function() {
        ...
        var datatable = $('#memberRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.KTStatusUpdate', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${window.location.protocol}//${window.location.host}/member/status-change/${elemID}`;
                }
            });
        })
        ...
        $(document).on('click', '.ktDelete', function() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `${location.protocol}//${window.location.host}/member/delete/${elemID}`;
                }
            });
        });
    };
