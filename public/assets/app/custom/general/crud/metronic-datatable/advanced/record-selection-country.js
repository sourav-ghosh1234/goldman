...
var KTDatatableCms = function() {
	...
	var options = {
		...
		data: {
			...
			source: {
				read: {
					url: `${location.protocol}//${window.location.host}/country/getall`,
					//url: `http://${window.location.host}/admin/country/getall`,
				},
			},
			pageSize: 10,
			serverPaging: true,
    ...
    var cmsSelector = function() {
        ...
        var datatable = $('#countryRecordSelection').KTDatatable(options);
        ...
        $(document).on('click', '.ktManagerDelete', function(){
            ...
            swal.fire({
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/country/delete/${elemID}`;
                }
            });
        }); 
        ...
        $(document).on('click', '.KTLanguageStatusUpdate', function(){
            ...
            swal.fire({
               ...
               // text: "You won't be able to revert this!",
                ...
                if (result.value) {
                    window.location.href = `http://${window.location.host}/country/status-change/${elemID}`;
                }
            });
        })
    };
