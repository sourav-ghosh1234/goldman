...
const BuyContent = require('real_estate/models/buy_real_estate_content.model');
...
const RealEstateRepository = {
    ...
    getAllBuyContent: async (req) => {
        try {
            ...
            let allbuycontents = await BuyContent.aggregatePaginate(aggregate, options);
            return allbuycontents;
        } catch (e) {
            throw (e);
        }
    },
    getBuyContentById: async(id) => {
        let result = await BuyContent.findById(id).lean().exec();
        try {
            if (!result) {
                return null;
            }
    ...
    getbuyContent: async(params) => {
        let result = await BuyContent.findOne(params).lean().exec();
        try {
            if (!result) {
                return null;
            }
    ...
    updateBuyContentById: async(data, id) => {
        try {
            let result = await BuyContent.findByIdAndUpdate(id, data, { new: true, upsert: true }).exec();
            if (!result) {
                return null;
            }
            return result;
    ...
    saveBuyContent: async (data) => {
        try {
            let save = await BuyContent.create(data);
            if (!save) {
                return null;
            }
            return save;
